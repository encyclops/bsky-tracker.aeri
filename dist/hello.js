...
// await agent.deleteFollow(uri);
const aBtn = document.getElementById('findNotFollowingBack');
const bBtn = document.getElementById('findNotYouFollowBack');
const cBtn = document.getElementById('findMutuals');
const dBtn = document.getElementById('findFollowers');
const eBtn = document.getElementById('findFollows');
...
const identifierInput = document.getElementById('floatingInput');
...
async function fetchFollowersWithPagination(limit = 100) {
    try {
        ...
        while (hasNextPage) {
            ...
            const url = `https://public.api.bsky.app/xrpc/app.bsky.graph.getFollowers?actor=${actor}&limit=${limit}${cursorFers ? `&cursor=${cursorFers}` : ""}`;
            // Make the API request
            const response = await fetch(url);
            // Check if the response is OK (status code 200-299)
            if (response.status === 400) {
...
async function fetchFollowsWithPagination(limit = 100) {
    try {
        ...
        while (hasNextPage) {
            // Construct the URL with query parameters
            const url = `https://public.api.bsky.app/xrpc/app.bsky.graph.getFollows?actor=${actor}&limit=${limit}${cursorFs ? `&cursor=${cursorFs}` : ""}`;
            // Make the API request
            const response = await fetch(url);
            // Check if the response is OK (status code 200-299)
            if (response.status === 400) {
...
async function findUnmatchedData() {
    ...
    if (followers && follows) {
        ...
        const mutuals = followers.filter((follower) => followDids.includes(follower.did));
        const unmatchedFollowers = followers.filter((follower) => !followDids.includes(follower.did));
        const unmatchedFollows = follows.filter((follow) => !followerDids.includes(follow.did));
        return {
            mutuals,
            unmatchedFollowers,
            unmatchedFollows,
...
function generateAllUserDataHTML(dataArray, allowed) {
    let html = "";
    if (dataArray) {
    ...
    userLinks.forEach((link) => {
        link.addEventListener("click", function (event) {
            ...
            const element = link; // Ensure link is treated as an HTMLElement
...
aBtn.onclick = async function () {
    ...
    if (data)
        generateAllUserDataHTML(data.unmatchedFollows, true);
    loaderDiv.style.display = "none";
};
bBtn.onclick = async function () {
    ...
    const data = await findUnmatchedData();
    if (data)
        generateAllUserDataHTML(data.unmatchedFollowers, false);
    loaderDiv.style.display = "none";
};
cBtn.onclick = async function () {
    ...
    const data = await findUnmatchedData();
    if (data)
        generateAllUserDataHTML(data.mutuals, true);
    loaderDiv.style.display = "none";
};
dBtn.onclick = async function () {
    ...
    if (data)
        generateAllUserDataHTML(data, false);
    loaderDiv.style.display = "none";
};
eBtn.onclick = async function () {
    ...
    if (data)
        generateAllUserDataHTML(data, true);
    loaderDiv.style.display = "none";
};
